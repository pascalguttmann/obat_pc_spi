if __name__ == "__main__":
    ...
    input(
        "Press enter to continue... with 144W Peltier Element, 50W Heater and Fan 100%"
    )
    device.write_fan(fan_speed_duty_cycle=1).wait()
    device.write_peltier(
        peltier_power=144, peltier_polarity=EnclosurePeltierPolarity.heating
    ).wait()
    ...
    device.write_peltier(
        peltier_power=0, peltier_polarity=EnclosurePeltierPolarity.heating
    ).wait()
    device.write_heater(heater_power=0)
