if __name__ == "__main__":
    ...
    for volts in [0.0, 2.0, 2.5, 5.0, 0.0]:
        input(f"Write PWM inputs to {volts} V. Press enter to continue...\n")
        device.get_dac().write(addr=0, voltage=volts)
        device.get_dac().write(addr=1, voltage=volts)
        device.get_dac().write(addr=2, voltage=volts)
